...
test("configuration is usable while MCP is off and does not start the listener", async () => {
  const app = await bootApp({ handlers: {
    get_mcp_connection_info: () => ({ command: "/Applications/Sodilaud.app/Contents/MacOS/sodilaud", args: ["--mcp-stdio"] }),
    start_mcp_server: () => ({ command: "/Applications/Sodilaud.app/Contents/MacOS/sodilaud", args: ["--mcp-stdio"] })
  } });
  ...
  assert.match(document.getElementById("mcp-config-command").value, /Sodilaud/);
  ...
  await settle();
  assert.deepEqual(JSON.parse(copied[0]).mcpServers.sodilaud.args, ["--mcp-stdio"]);
